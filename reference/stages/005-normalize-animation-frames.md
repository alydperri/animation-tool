...
For presentation GIFs or other baked in-frame motion, normalization may be
limited to the X axis. In x-only mode, center the subject horizontally while
preserving each frame's original vertical placement so jumps, hops, falls, and
other height changes remain visible in the exported GIF.
...
Once approved, run `reference/stages/006-finalize-animation-assets.md`
automatically.
