...
`reference/stages/001-generate-spritesheet.md`, with these batch-specific changes:

- Generate exactly `generation_count` candidate spritesheets.
- Do not ask the user to approve individual candidates.
- The image generation step must use ChatGPT/OpenAI image generation or an
  equivalent image-generation tool supplied by the active agent environment.
...
```text
reference/stages/batch/002-batch-stack-rank-spritesheets.md
```
